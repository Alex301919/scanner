...
from scanner.io import e, info, warn, error

class Target:

	def __init__(self, address, ip, ipversion, type, scanner):
	...
	def extract_service(self, line, regex=None):
		return self.scanner.extract_service(line, regex)

	async def extract_services(self, stream, regex=None):
		return await self.scanner.extract_services(stream, regex)
	...
	async def execute(self, cmd, blocking=True, outfile=None, errfile=None, future_outfile=None):
		...
		info('Сканирование порта {bblue}' + plugin.name + ' {green}(' + tag + '){rst} выполняет следующую команду по цели {byellow}' + address + '{rst}: ' + cmd, verbosity=2)
		...
		process, stdout, stderr = await target.scanner.execute(cmd, target, tag, patterns=plugin.patterns, outfile=outfile, errfile=errfile)
...
class Service:
	...
	async def execute(self, cmd, blocking=True, outfile=None, errfile=None, future_outfile=None):
		...
		info('Сканирование сервиса {bblue}' + plugin.name + ' {green}(' + tag + '){rst} выполняет следующую команду по цели {byellow}' + address + '{rst}: ' + cmd, verbosity=2)
