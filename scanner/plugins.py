...
from scanner.io import slugify, info, warn, error, fail, CommandStreamReader
from scanner.targets import Service
...
class Plugin(object):
	...
	def add_option(self, name, default=None, help=None):
		self.scanner.add_argument(self, name, metavar='VALUE', default=default, help=help)

	@final
	def add_constant_option(self, name, const, default=None, help=None):
		self.scanner.add_argument(self, name, action='store_const', const=const, default=default, help=help)

	@final
	def add_true_option(self, name, help=None):
		self.scanner.add_argument(self, name, action='store_true', help=help)

	@final
	def add_false_option(self, name, help=None):
		self.scanner.add_argument(self, name, action='store_false', help=help)

	@final
	def add_list_option(self, name, default=None, help=None):
		self.scanner.add_argument(self, name, nargs='+', metavar='VALUE', default=default, help=help)

	@final
	def add_choice_option(self, name, choices, default=None, help=None):
		if not isinstance(choices, list):
			fail('Выбранный аргумент для ' + self.name + '\'s ' + name + ' должен быть списком.')
		self.scanner.add_argument(self, name, choices=choices, default=default, help=help)
	...
	def get_option(self, name, default=None):
		...
		if name in vars(self.scanner.args):
			if vars(self.scanner.args)[name] is None:
				if default:
					return default
				else:
					return None
			else:
				return vars(self.scanner.args)[name]
		else:
			if default:
				return default
			return None
	...
	def add_pattern(self, pattern, description=None):
		try:
			...
			fail('Ошибка: шаблон "' + pattern + '" в плагине "' + self.name + '" имеет недопустимое регулярное выражение.')
	...
	def match_service_name(self, name, negative_match=False):
		...
		for r in name:
			try:
				...
				print('Некорректное регулярное выражение: ' + r)
				valid_regex = False
...
class Scanner(object):
	...
	async def extract_services(self, stream, regex):
		if not isinstance(stream, CommandStreamReader):
			print('Ошибка: в extract_services должен быть передан экземпляр CommandStreamReader.')
			sys.exit(1)
	...
	def register(self, plugin, filename):
		...
		if plugin.name is None:
			fail('Ошибка: Плагин с именем класса "' + plugin.__class__.__name__ + '" в ' + filename + ' не имеет имени.')

		for _, loaded_plugin in self.plugins.items():
			if plugin.name == loaded_plugin.name:
				fail('Ошибка: повторяющееся имя плагина "' + plugin.name + '" обнаружено в ' + filename + '.', file=sys.stderr)

		if plugin.slug is None:
			plugin.slug = slugify(plugin.name)
		elif not self.__slug_regex.match(plugin.slug):
			fail('Ошибка: предоставленный слаг "' + plugin.slug + '" в ' + filename + ' недействителен (должен содержать только строчные буквы, цифры и дефисы).', file=sys.stderr)

		if plugin.slug in config['protected_classes']:
			fail('Ошибка: слаг плагина "' + plugin.slug + '" в ' + filename + ' является защищенной строкой. Пожалуйста измените.')

		if plugin.slug not in self.plugins:

			for _, loaded_plugin in self.plugins.items():
				if plugin is loaded_plugin:
					fail('Ошибка: плагин "' + plugin.name + '" в ' + filename + ' уже загружен как "' + loaded_plugin.name + '" (' + str(loaded_plugin) + ')', file=sys.stderr)
			...
			for member_name, member_value in inspect.getmembers(plugin, predicate=inspect.ismethod):
				if member_name == 'configure':
					...
					if len(inspect.getfullargspec(member_value).args) != 2:
						fail('Ошибка: "запущенная" сопрограмма в плагине "' + plugin.name + '" с именем ' + filename + ' должна иметь два аргумента.', file=sys.stderr)
					run_coroutine_found = True
				elif member_name == 'manual':
					if len(inspect.getfullargspec(member_value).args) != 3:
						fail('Ошибка: "ручная" функция в плагине "' + plugin.name + '" с именем ' + filename + ' должна иметь два аргумента.', file=sys.stderr)
					manual_function_found = True

			if not run_coroutine_found and not manual_function_found:
				fail('Ошибка: плагин "' + plugin.name + '" с именем ' + filename + ' нужна либо «ручная» функция, либо «запуск» сопрограммы, либо и то, и другое.', file=sys.stderr)

			if issubclass(plugin.__class__, PortScan):
				if plugin.type is None:
					fail('Ошибка: плагин PortScan "' + plugin.name + '" с именем  ' + filename + ' требуется тип (либо tcp, либо udp).')
				else:
					plugin.type = plugin.type.lower()
					if plugin.type not in ['tcp', 'udp']:
						fail('Ошибка: плагин PortScan "' + plugin.name + '" с именем ' + filename + ' имеет недопустимый тип (должен быть tcp или udp).')
				...
				fail('Плагин "' + plugin.name + '" с именем ' + filename + ' не является ни PortScan, ни ServiceScan, ни Report.', file=sys.stderr)
			...
			fail('Ошибка: слаг плагина "' + plugin.slug + '" с именем ' + filename + ' уже назначен.', file=sys.stderr)
