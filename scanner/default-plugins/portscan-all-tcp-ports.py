...
class AllTCPPortScan(PortScan):
	...
	async def run(self, target):
		...
		while True:
			...
			if line is not None:
				match = re.search('^Обнаружен открытый порт ([0-9]+)/tcp', line)
				if match:
					target.info('Обнаружен открытый порт {bmagenta}tcp/' + match.group(1) + '{rst} на {byellow}' + target.address + '{rst}', verbosity=1)
