...
class Enum4Linux(ServiceScan):
	...
	def configure(self):
		self.add_choice_option('tool', default=('enum4linux-ng' if which('enum4linux-ng') else 'enum4linux'), choices=['enum4linux-ng', 'enum4linux'], help='Инструмент для перечисления Windows и Samba. Обычно: %(default)s')
	...
	def check(self):
		...
		if tool == 'enum4linux' and which('enum4linux') is None:
			self.error('Не удалось найти программу enum4linux. Убедитесь, что он установлен. (В Kali запустите: sudo apt install enum4linux)')
			return False
		elif tool == 'enum4linux-ng' and which('enum4linux-ng') is None:
			self.error('Не удалось найти программу enum4linux-ng. Убедитесь, что он установлен. (https://github.com/cddmp/enum4linux-ng)')
			return False
