from scanner.plugins import PortScan
	...
	async def run(self, target):
		...
		if os.getuid() == 0 or config['disable_sanity_checks']:
			...
			while True:
				...
				if line is not None:
					match = re.search('^Обнаружен открытый порт ([0-9]+)/udp', line)
					if match:
						target.info('Обнаружен открытый порт {bmagenta}udp/' + match.group(1) + '{rst} на {byellow}' + target.address + '{rst}', verbosity=1)
			...
			target.error('Для сканирования UDP требуется запуск сканера с правами root.')
