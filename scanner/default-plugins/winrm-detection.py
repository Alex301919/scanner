...
from scanner.io import fformat
...
class WinRMDetection(ServiceScan):
	...
	async def run(self, service):
		...
		with open(filename, mode='wt', encoding='utf8') as winrm:
			winrm.write('Возможно, WinRM был обнаружен работающим на ' + service.protocol + ' порт ' + str(service.port) + '.\nПроверьте _manual_commands.txt для ручных команд, которые вы можете запустить для этой службы.')
